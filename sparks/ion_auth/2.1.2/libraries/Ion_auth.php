...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user) 
			{
				...
				$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - '. lang('web_auth_voc'));
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - '. lang('web_auth_np'));
			$this->ci->email->message($message);
	...
	public function register($username, $password, $email, $additional_data = array(), $group_name = array()) //need to test email activation
	{
		...
		else
		{
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - '. lang('web_auth_act'));
			$this->ci->email->message($message);
